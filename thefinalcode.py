...
user_sessions = {}  # Dictionary to track active sessions
...
async def classplus_download(app: app, message: Message):
    ...
    if user_sessions.get(user_id, {}).get("active", False):
        await app.send_message(user_id, "⚠️ **आप पहले से ही एक सत्र में हैं! कृपया उसे पूरा करें या कुछ देर बाद पुनः प्रयास करें।**")
        return

    # ✅ Initialize session for the user
    user_sessions[user_id] = {"step": 1, "active": True}

    try:
        editable = await app.send_message(user_id, "🚀 **अपना Organization ID दर्ज करें**")

        # ✅ Custom Flow Function for Taking User Input
        async def get_user_input():
            while True:
                input_msg = await app.listen(user_id)  # No filters used
                if input_msg.chat.id == user_id:  # ✅ Ensuring only the original user can input
                    return input_msg

        # ✅ Get Organization ID
        org_msg = await get_user_input()
        orgid = org_msg.text.strip()
        await org_msg.delete(True)

        # ✅ Store in session
        user_sessions[user_id]["orgid"] = orgid
        user_sessions[user_id]["step"] = 2  # Move to next step
        ...
        text = await app.send_message(user_id, extracting_prompt)
        ...
        # ✅ Get Batch IDs (Ensuring only the same user can send input)
        bid_msg = await get_user_input()
        bid_input = bid_msg.text.strip()
        await bid_msg.delete(True)

        # Validate session step
        if user_sessions.get(user_id, {}).get("step") != 3:
            await app.send_message(user_id, "⚠️ **Session Error! कृपया फिर से शुरू करें।**")
            return
        ...
        user_sessions[user_id]["step"] = 4  # Move to next step
        ...
        for bid in batch_ids:
            try:
                ...
                thumb_p = await download_image(thu) if thu and thu != "#" else image

                await app.send_photo(chat_id=user_id, photo=thumb_p, caption=f"Processing batch: {bname}")
                ...
                if fetched_contents:
                    ...
                    output_file2 = f"{bname}_scammer.txt"
                ...
                await app.send_document(user_id, document=output_file2, caption=caption, thumb=image)
                ...
                await app.send_message(user_id, f"⚠️ **Error in Batch {bid}:** {str(e)}")

        await editable.delete(True)
        await app.send_message(user_id, "🎉 **Extraction Successful!**")

    except asyncio.TimeoutError:
        await app.send_message(user_id, "⌛ **Timeout! आपने बहुत देर कर दी। सत्र बंद किया जा रहा है।**")
    except Exception as e:
        await app.send_message(user_id, f"❌ **Error:** {str(e)}")
    finally:
        user_sessions.pop(user_id, None)  # Remove user session after completion
  # Clear session after completion   isme user sesiion sahi hai kya filter.userid add kro
